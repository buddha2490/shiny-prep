...
**2. Skill library — `.claude/skills/`.** Each skill states the correct house-standard pattern for a task — read the skill that governs the buggy code so your fix restores the intended pattern rather than inventing a new one. Most relevant: `reactive-programming` (reactive loops, over-firing, invalidation), `shiny-modules` (namespace/ID mismatches), `shiny-profiling` (reactlog/profvis to locate the fault), `shiny-error-handling` (req/validate/tryCatch selection, the catalog + incident-id framework), and the framework skills (`golem-app`, `leprechaun-app`, `rhino-app`) for deployment failures.
